...
async def scrape_kickass_anime():
    ...
    dengan selector yang benar dan penanganan error yang lebih tangguh.
    """
    async with async_playwright() as p:
        ...
        try:
            ...
            # Kita ambil 5 anime pertama untuk demonstrasi
            for index, item in enumerate(anime_items[:5]):
                print(f"\n--- Memproses Item #{index + 1} ---")
                detail_page = None # Inisialisasi agar bisa dicek di blok except
                try:
                    # 1. Ambil URL Poster (logika ini sudah bagus)
                    ...
                    # 2. [PERBAIKAN UTAMA] Ambil URL halaman detail dari link judul, bukan dari kartu
                    ...
                    # 3. Buka halaman detail dan ekstrak data
                    detail_page = await context.new_page()
                    await detail_page.goto(full_detail_url, timeout=60000)
                    
                    # [PERBAIKAN KETAHANAN] Tunggu hingga halaman selesai memuat struktur dasarnya
                    await detail_page.wait_for_load_state('domcontentloaded', timeout=60000)

                    # Sekarang cari elemennya
